...
# Without the EBS CSI driver, PVCs that fall back to the default `gp2`
# StorageClass (provisioner: kubernetes.io/aws-ebs) stay Pending on EKS
# 1.23+ — the in-tree provisioner is migrated to CSI. The orchestration
# chart's spark-history-server PVC depends on this.
